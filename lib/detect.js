...
export function lspPluginFor(detection) {
  const fw = new Set(detection.frameworks);
  const stack = detection.stack;
  if (fw.has('typescript') || stack === 'nextjs' || stack === 'react') return 'typescript-lsp';
  if (stack === 'node-api') return 'typescript-lsp';
  if (stack === 'python-api') return 'pyright-lsp';
  if (stack === 'rust') return 'rust-analyzer-lsp';
  if (stack === 'go') return 'gopls-lsp';
  return null;
}
